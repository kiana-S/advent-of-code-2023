...
data Card = Joker | Two | Three | Four | Five | Six | Seven
...
countCards = sortBy (compare `on` snd) . SortedMap.toList . jokerify . go empty
  where
    ...
    jokerify : SortedMap Card Nat -> SortedMap Card Nat
    jokerify mp =
      case lookup Joker mp of
        Nothing => mp
        Just n =>
          let mp' = delete Joker mp
              (highest, v) =
                foldr (\(c,x),(c',y) =>
                      if x < y then (c',y) else (c,x))
                  (Joker, 0) (SortedMap.toList mp')
          in  insert highest (n + v) mp'
